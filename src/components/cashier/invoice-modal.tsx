...
export function InvoiceModal({ bill, isOpen, onClose }: InvoiceModalProps) {
  ...
  const generateCustomerCopyHtml = () => {
    ...
    return `
      <html>
        <head>
          <title>Al-Shawaya Customer Invoice</title>
  ...
  const generateKitchenCopyHtml = () => {
    ...
    return `
      <html>
        <head>
          <title>Al-Shawaya Kitchen Order</title>
          <style>${thermalPrintStyles}</style>
        </head>
        <body class="thermal-print-body">
  ...
  const handlePrint = (htmlContent: string, copyType: 'Customer' | 'Kitchen') => {
    ...
    if (printWindow) {
      ...
      // Delay slightly to ensure content is fully loaded before printing
      setTimeout(() => {
        try {
          ...
          toast({
            ...
            description: `The ${copyType.toLowerCase()} copy is being processed by your browser's print dialog.`,
          });
          // Note: Closing the window immediately might cancel the print job in some browsers.
          // Consider printWindow.onafterprint = () => printWindow.close(); if you want to auto-close.
        } catch (error) {
          console.error("Error printing:", error);
          toast({
            title: "Print Error",
            description: `Could not initiate printing for the ${copyType.toLowerCase()} copy. Check console for details.`,
            variant: "destructive",
          });
        }
      }, 250); // Adjusted timeout for content loading
    } else {
      toast({
        title: "Popup Blocked",
        description: `Could not open print window for ${copyType.toLowerCase()} copy. Please allow popups for this site.`,
        variant: "destructive",
      });
    }
  };
